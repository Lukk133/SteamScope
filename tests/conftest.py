...
def views_conn(tmp_path):
    ...
    init_schema(conn)
    stage_kaggle(FIXTURES_DIR / "etl_sample_kaggle.csv", conn)
    stage_steam_api(FIXTURES_DIR / "etl_sample_steam_api", conn)
    stage_steamspy(FIXTURES_DIR / "etl_sample_steamspy", conn)
    stage_reviews(FIXTURES_DIR / "etl_sample_reviews", conn)
